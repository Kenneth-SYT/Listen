...
type AdminView = 'bookings' | 'users' | 'management'
  ...
  return <section className="admin-page">
    ...
    {allowed && <>
      ...
      <div className="admin-toolbar">
        <nav aria-label="Admin sections">{(['bookings', 'users', 'management'] as AdminView[]).map(item => <button key={item} type="button" className={view === item ? 'active' : ''} aria-pressed={view === item} onClick={() => { setView(item); setSearch('') }}>{item === 'bookings' ? 'Bookings' : item === 'users' ? 'Users' : 'Management'}</button>)}</nav>
        {view !== 'management' && <label className="admin-search"><Search size={17} /><span className="sr-only">Search</span><input value={search} onChange={event => setSearch(event.target.value)} placeholder={view === 'users' ? 'Search users' : 'Search bookings'} /></label>}
      </div>
      ...
      {view === 'management' && <section className="admin-section"><div className="admin-section-heading"><div><h2>Booking management</h2><p>Manage consultant access, availability and pricing. Dates use {Intl.DateTimeFormat().resolvedOptions().timeZone}.</p></div></div><div className="admin-management-grid">
