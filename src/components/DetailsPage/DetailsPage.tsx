...
function DetailsPage({ initialDetails, selection, answers, onBack, onDetailsChange }: Props) {
  ...
  const visibleMessage = verifiedEmail && /confirm your email|check your email/i.test(message) ? '' : message
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault()
    if (busy || loading || (!session && retrySeconds > 0) || holdSeconds <= 0 || !holdValid || holdChecking || (!session && accountMode === 'signup' && awaitingConfirmation)) return
  ...
  return <>
    <div className="details-layout"><form className="details-form" onSubmit={submit}>
      <header className="details-heading"><span className="match-eyebrow">Final step before payment</span><h1 id="match-heading">{signinOnly ? 'Welcome back.' : accountCreatedThisVisit ? 'Your account is ready.' : session ? 'Complete your booking.' : 'Create your account.'}</h1><p>{signinOnly ? 'Sign in to continue with the time you selected.' : accountCreatedThisVisit ? 'Your email is verified. Review your booking and proceed when you’re ready.' : 'We’ll use these details to prepare your booking and contact you about your appointment.'}</p>
      ...
      <div className="details-actions"><button type="button" className="details-back" onClick={backToBooking} disabled={busy}><ArrowLeft size={18} /> Back to times</button><button type="submit" disabled={busy || loading || !supabase || (!session && retrySeconds > 0) || holdSeconds <= 0 || !holdValid || holdChecking || duplicateEmailEntered || (!session && accountMode === 'signup' && awaitingConfirmation)}>{busy ? <><LoaderCircle className="details-button-spinner" size={18} aria-hidden="true" /> Opening secure payment…</> : <>{session ? 'Proceed to payment' : awaitingConfirmation && accountMode === 'signup' ? 'Verify your email above' : accountMode === 'signup' ? 'Create account and continue' : 'Sign in and continue'} <ArrowRight size={18} /></>}</button></div>
