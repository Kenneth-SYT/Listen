...
// deliberate local testing session.
const emailVerificationRequired = import.meta.env.VITE_REQUIRE_EMAIL_VERIFICATION !== 'false'
...
function DetailsPage({ initialDetails, selection, answers, onBack, onDetailsChange, onAccountReady }: Props) {
  ...
  const [holdSeconds, setHoldSeconds] = useState(selection ? 0 : 1)
  const [holdValid, setHoldValid] = useState(!selection)
  const [holdChecking, setHoldChecking] = useState(!!selection)
  ...
  useEffect(() => {
    if (!session?.user.id || !selection) return
    let active = true
  ...
  const backToBooking = async () => {
    if (selection) await getSupabase().rpc('release_slot_hold', { p_token: selection.holdToken })
    onBack()
  }
  ...
  const verifyEmailCode = async () => {
    if (busy || emailOtp.length < 6) return
    setBusy(true); setMessage(''); setCheckoutError('')
    try {
      const email = details.email.trim().toLowerCase()
      const { data, error } = await getSupabase().auth.verifyOtp({ email, token: emailOtp, type: 'email' })
      if (error) throw error
    ...
    try {
      ...
      if (!session) {
        ...
        if (currentAuth.session) {
          setMessage('You are already signed in. Continue with this account or choose “Use another account”.')
          return
        }
        if (accountMode === 'signup') {
          ...
          // With Confirm email disabled, Supabase signs a new user in and marks
          // the address confirmed immediately. Fail closed so a dashboard
          // configuration mistake cannot bypass ownership verification.
          if (emailVerificationRequired && data.session) {
            await client.auth.signOut({ scope: 'local' })
            setMessage('Email verification is not enabled in Supabase. Turn on Confirm email under Authentication → Sign In / Providers → Email, then create the account again.')
            return
          }
          if (!data.session) {
          ...
          if (error) throw error
          setMessage(selection ? 'Signed in. Review your details, then continue to payment.' : 'Signed in. Review your details, then continue to choose a listener and time.')
          return
        }
      }
  ...
  return <>
    <div className={`details-layout${selection ? '' : ' details-layout-account'}`}><form className="details-form" onSubmit={submit}>
      ...
      {passwordMismatch && <p id="details-password-error" role="alert" className="details-message details-message-error">Passwords do not match. Please enter the same password in both fields.</p>}
      {!session && awaitingConfirmation && <section className="details-email-otp" aria-labelledby="email-code-heading">
    ...
    {selection && displayQuote && <aside className="details-review"><h2>Review your booking</h2><div className="details-hold" role="status"><strong>{holdSeconds > 0 ? holdTime : 'Hold expired'}</strong><span>{holdSeconds > 0 ? holdChecking ? 'Checking your hold…' : holdValid ? 'This time is held for you' : 'Could not verify this hold' : 'Choose a new time to continue'}</span></div><dl><div><dt>Listener</dt><dd>{selection.listener.name}</dd></div><div><dt>Time</dt><dd>{new Date(selection.slot.starts_at).toLocaleString('en-AU')}</dd></div><div><dt>Session</dt><dd>{displayQuote.label} · {displayQuote.duration_minutes} minutes</dd></div><div><dt>Questionnaire</dt><dd>{answers.questionnaire === 'long' ? 'Long questionnaire' : 'Short check-in'}</dd></div><div><dt>Topics</dt><dd>{answers.topics.join(', ')}</dd></div><div><dt>Listener preference</dt><dd>{answers.listenerGender}</dd></div><div><dt>{wellbeingScoreLabel(answers)}</dt><dd>{wellbeingScore(answers)} / {answers.questionnaire === 'long' ? 50 : 24}</dd></div>{answers.note && <div><dt>Your note</dt><dd>{answers.note}</dd></div>}</dl><p><strong>Estimated rate: {money(displayQuote.amount_cents)} AUD</strong></p><p>Complete your account within the hold time. After checkout starts, Stripe gives you its own payment window.</p></aside>}</div>
